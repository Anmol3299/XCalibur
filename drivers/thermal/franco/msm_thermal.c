 ...
 * 7. Added a function to allow users to configure whether Core 0 should be disabled or not.
 * 8. Added a check to make sure that Frequency Input from the user is only taken when Permission to Disable Core has not been granted even once (for big.LITTLE SoC) to prevent freezes.
 * 9. Introduced Shoaib's Core Control, an Automatic HotPlug based on Temperature.
 * 10. Updated Shoaib's Core Control to v2.0 with Improvements and BUG-Fixes as well as Support for Hexa-Core big.LITTLE SoCs.
 * 11. Altered the Formatting of the Codes (looks cleaner and more beautiful now).
 *
