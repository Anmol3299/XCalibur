...
static void check_temp(struct work_struct *work)
{
	...
	if (temp > (temp_threshold + 3*temp_step) && freq_hell <= info.cpuinfo_max_freq)
		freq = freq_hell;
	else if (temp > (temp_threshold + 2*temp_step) && freq_very_hot <= info.cpuinfo_max_freq)
		freq = freq_very_hot;
	else if (temp > (temp_threshold + temp_step) && freq_hot <= info.cpuinfo_max_freq)
		freq = freq_hot;
	else if (temp > temp_threshold && freq_warm <= info.cpuinfo_max_freq)
		freq = freq_warm;
	else
